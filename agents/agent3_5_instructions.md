...
<mission>
Scan the merchant's available food photos, match them to each occasion's hero food item from Agent 3's output, and write a photo-to-occasion mapping file that Agent 4 reads to optionally upload real photos alongside Nano Banana Pro generation prompts.
</mission>
...
<photo-sources priority-order="true">
...
<phase n="1" name="Parse Agent 3 Content">
...
3. **For each occasion, extract:**
   ...
   - `hero_food_keywords` -- extract the specific menu item name(s) from the food description. Examples:
     ...
     - "Pernil" or "Pernil with Rice" → keywords: **Pernil**
...
<phase n="2" name="Scrape Photos from DoorDash Storefront (Primary)">
...
<step n="2.1" name="Navigate to the Storefront">
...
<step n="2.2" name="Pull the Merchant Logo">
...
<step n="2.3" name="Scroll and Collect Menu Item Images">
...
<step n="2.4" name="Build the Storefront Photo Inventory">
...
<step n="2.5" name="Download Matched Photos">
...
<phase n="2b" name="Scan Local Photo Folders (Backup)">
...
<phase n="3" name="Match Occasions to Photos">
For each occasion extracted in Phase 1:

<step n="3a" name="Keyword Matching">
...
<step n="3b" name="Visual Selection (when a match is found)">
...
<step n="3c" name="Record the Match">
...
<phase n="4" name="Stage Reference Photos for Agent 4">
...
<selection-priority>
When multiple candidate photos exist, prefer (in order):
...
- Photo source folder(s) OR DoorDash storefront via Playwright
- Google Sheet `1N-snGtkhdYilf4DBMulQLnPkgOlXIHi3Rtkon04Qi28`, tab "Social Media" (for Business ID, column C)
</reads>
<writes>
- `merchants/{Merchant Name}/agent3_5_photos.md`
- Staged reference photos to `merchants/{Merchant Name}/reference_photos/`
</writes>
</file-io>
