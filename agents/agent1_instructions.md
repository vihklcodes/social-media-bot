...
Read the merchant's row from the **"Social Media"** tab of Google Sheet ID `1N-snGtkhdYilf4DBMulQLnPkgOlXIHi3Rtkon04Qi28` using `sheets_read`:
- **Column F (Website):** The merchant's website URL. Navigate to this URL directly with Playwright -- do NOT web search for the website.
- **Column E (Social Media Link):** The merchant's Instagram URL. Navigate to this URL directly with Playwright -- do NOT web search for the Instagram handle.
- **Column C (Business ID):** The merchant's DoorDash Business ID.
- **Column D (Business Name):** Confirm the merchant name matches.
To find the merchant's row, read `Social Media!A1:S14` and match on the Business Name in column D.
</data-source-rule>
...
**PRIMARY source for Agent 3's text design.**
...
```
mcp__playwright__browser_navigate → [URL from column E of Google Sheet]
```
...
<step n="2" name="Full-page screenshot">
Take a full-page screenshot to capture the entire visible grid:
```
...
<step n="3" name="DOM snapshot">
Capture the DOM snapshot to extract all captions, alt text, post URLs, and follower counts:
```
...
<step n="6" name="Visual analysis">
Visually analyze every screenshot to identify:
...
<section n="1" name="Visual Identity Breakdown">

<aesthetic-category>
Select primary + secondary based on what you SEE, not what you assume:
...
<text-placement-map critical="true">
**CRITICAL -- do NOT default to "bottom".** Document WHERE text appears across ALL posts that have text overlays. Create a placement map:
...
<text-background-treatment-map critical="true">
**CRITICAL -- do NOT assume one treatment for all posts.** Document the VARIETY of text background treatments across real posts:
...
<section n="3" name="Photography Style">
Observed from actual STATIC PHOTO posts -- focus on photos, not video stills. Prioritize last 3 months, then 6 months.
...
Do NOT use generic categories. Use **Playwright pixel-sampled hex codes** -- cross-reference with your canvas extraction data:
...
<section n="4" name="Graphic Design System">
Observed from actual posts:
...
<section n="5" name="Caption & Formatting Style" critical="true">
...
<section n="6" name="Real Caption Examples" required="true">
...
<section n="7" name="Brand Tone in 3 Words">
Derived from real captions, not assumptions:
